<script>
export default {
    name: 'AppJumbotron',
}
</script>

<template>
    <section id="jumbotron">

        <div class="jumbo_content d-flex justify-content-center align-items-center h-100">

            <div class="text-center w-25">

                <!-- NEED TO FIX FONT FAMILY -->
                <h2>Hello, Im Martin</h2>
                <!-- NEED TO FIX FONT FAMILY -->

                <div class="fw-bold fs-3">
                    Artist Coaching And Mentoring Might Be for You.
                </div>

                <a href="##" class="btn mt-3 btn_warning">
                    Get started today
                </a>

            </div>
        </div>


        <div class="shortcuts d-flex flex-column gap-2">

            <button class="btn demo">
                <svg xmlns="http://www.w3.org/2000/svg" height="1em" viewBox="0 0 512 512">
                    <path
                        d="M177.9 494.1c-18.7 18.7-49.1 18.7-67.9 0L17.9 401.9c-18.7-18.7-18.7-49.1 0-67.9l50.7-50.7 48 48c6.2 6.2 16.4 6.2 22.6 0s6.2-16.4 0-22.6l-48-48 41.4-41.4 48 48c6.2 6.2 16.4 6.2 22.6 0s6.2-16.4 0-22.6l-48-48 41.4-41.4 48 48c6.2 6.2 16.4 6.2 22.6 0s6.2-16.4 0-22.6l-48-48 41.4-41.4 48 48c6.2 6.2 16.4 6.2 22.6 0s6.2-16.4 0-22.6l-48-48 50.7-50.7c18.7-18.7 49.1-18.7 67.9 0l92.1 92.1c18.7 18.7 18.7 49.1 0 67.9L177.9 494.1z" />
                </svg>
                <div>
                    Select Demo
                </div>
            </button>

            <button class="btn support">
                <svg xmlns="http://www.w3.org/2000/svg" height="1em" viewBox="0 0 512 512">
                    <path
                        d="M337.8 205.7l48.6-42.5c13.8 19.3 23.4 41.9 27.4 66.2l-64.4 4.3c-2.4-10.1-6.4-19.5-11.6-28zm140.1 19.5c-5.3-38.8-20.6-74.5-43.2-104.3l.8-.7C449 108.4 449.7 87.6 437 75s-33.4-12-45.2 1.5l-.7 .8c-29.8-22.6-65.5-37.9-104.3-43.2l.1-1.1c1.2-17.9-13-33-30.9-33s-32.1 15.2-30.9 33l.1 1.1c-38.8 5.3-74.5 20.6-104.3 43.2l-.7-.8C108.4 63 87.6 62.3 75 75s-12 33.4 1.5 45.2l.8 .7c-22.6 29.8-37.9 65.5-43.2 104.3l-1.1-.1c-17.9-1.2-33 13-33 30.9s15.2 32.1 33 30.9l1.1-.1c5.3 38.8 20.6 74.5 43.2 104.3l-.8 .7C63 403.6 62.3 424.4 75 437s33.4 12 45.2-1.5l.7-.8c29.8 22.6 65.5 37.9 104.3 43.2l-.1 1.1c-1.2 17.9 13 33 30.9 33s32.1-15.2 30.9-33l-.1-1.1c38.8-5.3 74.5-20.6 104.3-43.2l.7 .8c11.8 13.5 32.5 14.2 45.2 1.5s12-33.4-1.5-45.2l-.8-.7c22.6-29.8 37.9-65.5 43.2-104.3l1.1 .1c17.9 1.2 33-13 33-30.9s-15.2-32.1-33-30.9l-1.1 .1zM163.2 125.6c19.3-13.8 41.9-23.4 66.2-27.5l4.3 64.4c-10 2.4-19.5 6.4-28 11.6l-42.5-48.6zm-65 103.8c4.1-24.4 13.7-46.9 27.5-66.2l48.6 42.5c-5.3 8.5-9.2 18-11.6 28l-64.4-4.3zm27.5 119.4c-13.8-19.3-23.4-41.9-27.5-66.2l64.4-4.3c2.4 10 6.4 19.5 11.6 28l-48.6 42.5zm103.8 65c-24.4-4.1-46.9-13.7-66.2-27.4l42.5-48.6c8.5 5.3 18 9.2 28 11.6l-4.3 64.4zm119.4-27.4c-19.3 13.8-41.9 23.4-66.2 27.4l-4.3-64.4c10-2.4 19.5-6.4 28-11.6l42.5 48.6zm65-103.8c-4.1 24.4-13.7 46.9-27.4 66.2l-48.6-42.5c5.3-8.5 9.2-18 11.6-28l64.4 4.3zm-65-156.9l-42.5 48.6c-8.5-5.3-18-9.2-28-11.6l4.3-64.4c24.4 4.1 46.9 13.7 66.2 27.5zM256 224a32 32 0 1 1 0 64 32 32 0 1 1 0-64z" />
                </svg>

                <div>
                    Support Channel
                </div>
            </button>

            <button class="btn documentation">
                <svg xmlns="http://www.w3.org/2000/svg" height="1em" viewBox="0 0 448 512">
                    <path
                        d="M96 0C43 0 0 43 0 96V416c0 53 43 96 96 96H384h32c17.7 0 32-14.3 32-32s-14.3-32-32-32V384c17.7 0 32-14.3 32-32V32c0-17.7-14.3-32-32-32H384 96zm0 384H352v64H96c-17.7 0-32-14.3-32-32s14.3-32 32-32zm32-240c0-8.8 7.2-16 16-16H336c8.8 0 16 7.2 16 16s-7.2 16-16 16H144c-8.8 0-16-7.2-16-16zm16 48H336c8.8 0 16 7.2 16 16s-7.2 16-16 16H144c-8.8 0-16-7.2-16-16s7.2-16 16-16z" />
                </svg>
                <div>
                    Documentation
                </div>
            </button>

            <button class="btn purchase">
                <svg xmlns="http://www.w3.org/2000/svg" height="1em" viewBox="0 0 576 512">
                    <path
                        d="M0 24C0 10.7 10.7 0 24 0H69.5c22 0 41.5 12.8 50.6 32h411c26.3 0 45.5 25 38.6 50.4l-41 152.3c-8.5 31.4-37 53.3-69.5 53.3H170.7l5.4 28.5c2.2 11.3 12.1 19.5 23.6 19.5H488c13.3 0 24 10.7 24 24s-10.7 24-24 24H199.7c-34.6 0-64.3-24.6-70.7-58.5L77.4 54.5c-.7-3.8-4-6.5-7.9-6.5H24C10.7 48 0 37.3 0 24zM128 464a48 48 0 1 1 96 0 48 48 0 1 1 -96 0zm336-48a48 48 0 1 1 0 96 48 48 0 1 1 0-96z" />
                </svg>
                <div>
                    Purchase Maxcoach
                </div>
            </button>
        </div>

    </section>
</template>

<style lang="scss" scoped>
@use '../assets/scss/partials/variables' as *;

#jumbotron {
    height: 740px;
    background-color: $mc_light;
    background-image: url('../assets/img/artist-hero-image-01.jpg'), url('../assets/img/artist-hero-image-02.jpg'), url('../assets/img/artist-hero-image-03.png'), url('../assets/img/artist-hero-image-04.jpg'), url('../assets/img/maxcoach-shape-12-150x150.png'), url('../assets/img/artist-shape-01.png'), url('../assets/img/artist-shape-02.png'), url('../assets/img/artist-shape-03.png'), url('../assets/img/artist-shape-04.png'), url('../assets/img/artist-shape-05.png'), ;
    background-position: left 145px bottom 60px, left 330px bottom 270px, left 480px bottom 25px, right 240px bottom 50px, right 400px bottom 35px, top right 160px, top left -300px, left 15px top 160px, top 80px right 150px, bottom right 90px;
    background-repeat: no-repeat;

    position: relative;

    .shortcuts {
        position: absolute;
        top: 4rem;
        right: 0;

        background-color: $mc_white;

        button {
            position: relative;

            div {
                display: none;
                align-items: center;
                justify-content: center;
                height: 100%;
                min-width: 200px;
                position: absolute;
                right: 100%;
                top: 0;
                border-radius: 0.5rem;


                background-color: $mc_dark;
                color: $mc_white
            }

        }

        button.demo:hover,
        button.support:hover,
        button.documentation:hover,
        button.purchase:hover {
            div {
                display: inline-flex;
            }
        }
    }

    .jumbo_content .btn_warning {
        padding: 0.5rem 2.25rem;
    }
}
</style>